...
public class PromptServiceImpl implements PromptService {
    ...
    public PromptServiceImpl(PromptRepository promptRepository, ConversionService conversionService) {
    ...
    public Prompt findPromptById(Integer id) {
        return convertPromptEntityToPrompt(promptRepository.findById(id).orElseThrow(() -> new RuntimeException("Couldn't find user.")));
    }

    @Override
    public Prompt findStoryByDate(LocalDate date) {
        return convertPromptEntityToPrompt(promptRepository.findByDate(date).orElseThrow(() -> new RuntimeException("Couldn't find user.")));
    }
